...
>>> parse('define f1 1 end define f2 2 end f1 f2')
[{'f1': [1], 'f2': [2]}, ['f1', 'f2']]
>>> parse('if 1 if 2 endif endif')
[{}, [['if', [1, ['if', [2]]]]]]
>>> parse('''define -- 1 - end
         define =0? dup 0 = end
         define =1? dup 1 = end
         ...
         4 factorial''')
[{'--': [1, '-'], '=0?': ['dup', 0, '='], '=1?': ['dup', 1, '='], 
'factorial': ['=0?', ['if', ['drop', 1], ['=1?', ['if', ['drop', 1], 
['dup', '--', 'factorial', '*']]]]]}, 
...
# Вывод
В ходе выполнения лабораторной работы я научился реализовывать
 синтаксический анализатор методом рекурсивного спуска для
 небольшого FORTH-подобного языка с поддержкой 
определений и условных конструкций. Было важно правильно 
обрабатывать вложенные структуры и стоп-слова, а также 
корректно сигнализировать об ошибках. Полученные навыки 
станут основой для дальнейшего изучения компиляции и разработки
 языков программирования.Возможность отправки отчёта разблокирована
